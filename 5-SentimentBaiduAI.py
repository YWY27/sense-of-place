...
class GetAccessToken:
    def __init__(self):
        ...
        self.AK = 'YtYOWhoKb1qZg9mWpXiNm0j2'
        self.SK = 'ZdoeajBkN6idDkxLBdLun4GppK8K3vUD'
...
class SentimentBaidu:
    ...
    def run(self):
        ...
        commentcontent = contentalldf

        result = pd.read_csv(r'E:\2-Codes\Project\bishe\data\cpbaidu.csv')
        ...
        # for comment in commentcontent['content']:
        for index,row in commentcontent.iterrows():
            if row['content'] and row['weibo_id'] not in result['weibo_id'].values:
                ...
                if commentsenti is not None:
                    commentbatch = [weibo_id]+[comment]+commentsenti
                    self.commentcomment.append(commentbatch)
                if self.count % 100 == 0:
                    commentsentidf = pd.DataFrame(self.commentcomment,
                                                columns=['weibo_id', 'content', 'contentposprob', 'contentnegprob',
                                                        'contentconfi', 'contentsenti'])
                    fpath = self.textpath + '/cpbaidu1.csv'
...
if __name__=='__main__':
    tp = r'E:\2-Codes\Project\bishe\data\weibo1' # 【修改点2】修改为自己的文本存储的文件夹位置
